...
void PhongMaterial::receiveParameter( int paramId, void* paramValue )
{
	switch( paramId )
	{
	case RT_TEXTURE_ID:
...
void PhongMaterial::shade( rts::RTstate& state )
{
	...
	rtu::float3 objColor;
	rtsComputeShadingColor( state, objColor );
	...
	returnColor.r = objColor.r * ( _ambient.r + diffuse.r ) + ( _specularColor.r * specular.r );
	returnColor.g = objColor.g * ( _ambient.g + diffuse.g ) + ( _specularColor.g * specular.g );
	returnColor.b = objColor.b * ( _ambient.b + diffuse.b ) + ( _specularColor.b * specular.b );
